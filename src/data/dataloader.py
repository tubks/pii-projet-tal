...
# def tokenize(example, tokenizer, overlap_size):
#     """
#     Tokenizes the input
#     To be used with datasets.map() with batched=False
...
def preprocess_data(data, tokenizer, label2id, overlap_size=0, keys_to_keep=[]):
    """
    Preprocesses the data
    
    Takes in 
        - data: a dataset object with columns ['document', 'tokens', 'labels']
    ...
    keys_to_flatten = list(set(['labels', 'input_ids', 'token_type_ids', 'attention_mask', 'org_word_ids', 'document'] + keys_to_keep))
...
if __name__=='__main__':
    ...
    data = preprocess_data(data, tokenizer, label2id)
